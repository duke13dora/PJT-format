...
const CALLOUT_BG = 'D3E2FF';          // コールアウトボックスの背景色

const templatePath = './templates/flux-presentation-template.pptx';  // テンプレートPPTX
const outputPath = './最終報告書_v1.pptx';                       // 出力先
...
// ※空テンプレートの場合はslideLayout XMLを直接使うか、別途ベーススライドを用意
...
// slides.push({
//   xml: buildContentSlide(contentTemplate, '01 エグゼクティブサマリ', '実施概要と成果',
//     'リード文...', 4, [
//       makeTable(n++, 0.4, 1.5, [0.4, 2.3, 6.5], ['#','項目','内容'], [['1','期間','XXX']], {}),
//     ]),
//   rels: contentRels
// });
...
if (slides.length === 0) {
  ...
  console.log('詳細: guides/08-final-report.md');
  process.exit(0);
}
