...
const members = await asanaApi('/workspaces/{workspace_gid}/workspace_memberships?opt_fields=user.name');
```
...
```javascript
const text = profileUrl('{MAIN_RECIPIENT_GID}')
  + '\ncc ' + profileUrl('{MEMBER2_GID}') + ' ' + profileUrl('{MEMBER3_GID}') + ' {手動名前}'
  + '\n\n'
  + '本文をここに記載';
```
...
await fetch('https://app.asana.com/api/1.0/tasks/' + task.data.gid + '/stories', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' },
  body: JSON.stringify({
    data: { text: profileUrl('{ASSIGNEE_GID}') + '\n\nMM/DD定例ネクスト：サブタスク名' }
  })
});
```
...
定例前に進捗確認・会話再開が必要なチケットにコメントを投げる運用。

### 毎週固定でつつくチケット（定型コメント）

プロジェクトで「毎週必ず確認すべきチケット」を定義し、定型コメントを投げる:
...
上記に加え、会話が止まっているチケットや今週特に確認が必要なものを追加。
